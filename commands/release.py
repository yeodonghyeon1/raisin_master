...
from commands.setup import setup, get_commit_hash, guard_require_version_bump_for_src_packages
...
def release(target, build_type):
    ...
    if not os.path.isdir(target_dir):
        print(f"❌ Error: Target '{target}' not found in '{os.path.join(g.script_directory, 'src')}'.")
        return
    ...
    try:
        with open(release_file_path, 'r') as file:
            ...
            if platform.system().lower() == "linux":
                ...
                subprocess.run(cmake_command, check=True, text=True, env=g.developer_env)
                print("✅ CMake configuration successful.")
                print("🛠️  Building with Ninja...")

                subprocess.run(
                    ["cmake", "--build", str(build_dir), "--parallel"],
                    check=True, text=True, env=g.developer_env
                )
            ...
            shutil.copy(Path(g.script_directory) / 'src' / target / 'release.yaml', Path(install_dir) / 'release.yaml')
            if (Path(g.script_directory) / 'src' / target / 'install_dependencies.sh').is_file():
                shutil.copy(Path(g.script_directory) / 'src' / target / 'install_dependencies.sh', Path(install_dir) / 'install_dependencies.sh')
